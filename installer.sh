...
surat=$(curl -sS https://raw.githubusercontent.com/kenDevXD/acces/main/ip | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/kenDevXD/bot_panel/master/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
